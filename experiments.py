...
from graphs import generateRandomGraph, verifyVertexCover
...
def avgRT(algorithm, n, p, trials, is_bb=False):
    # average runtime over several randomly generated graphs
    # returns None if any trial times out (bb only)
    times = []
    for seed in range(trials):
        graph, edges = generateRandomGraph(n, p, seed=seed * 31 + n)
        elapsed, cover = measureRT(algorithm, graph, n if is_bb else None)

        assert verifyVertexCover(cover, edges)
...
def experiment1():
    ...
    for n in range(5, 31, 2):
        t = avgRT(branchbound, n, p, NUM_TRIALS, is_bb=True)
        if t is None:
    ...
    ax.plot(bbSizes, bbTimes, "o-", color=BB_COLOR, linewidth=2, markersize=6, label="Branch-and-Bound")
...
def experiment2():
    ...
    for p in probs:
        bbTime = avgRT(branchbound, n, p, NUM_TRIALS, is_bb=True)
...
def experiment3():
    ...
    for n in sizes:
        ...
        for seed in range(TRIALS):
            ...
            assert verifyVertexCover(greedy_cover, edges)

            bbSize.append(len(bb_cover))
            gdSize.append(len(greedy_cover))
            
            if len(bb_cover) > 0:
                trialRatios.append(len(greedy_cover) / len(bb_cover))

        bbAvgs.append(statistics.mean(bbSize))
